...
class AnalyticsReportViewSet(viewsets.ModelViewSet):
    ...
    def generate(self, request):
        ...
        try:
            ...
            # 生成报表数据
            result_data = analytics_service.generate_report(report_type, parameters)
    ...
    def export(self, request, pk=None):
        ...
        try:
            ...
            if export_format == 'csv':
                ...
                response['Content-Disposition'] = f'attachment; filename="{report.title}_{timezone.now().strftime("%Y%m%d%H%M%S")}.csv"'
                ...
                response['Content-Disposition'] = f'attachment; filename="{report.title}_{timezone.now().strftime("%Y%m%d%H%M%S")}.json"'
                response.write(export_data)
                return response
