...
from mind_map.models import MindMapTemplate
from mind_map.mongodb_models import MindMapTemplate as MongoMindMapTemplate
from mind_map.serializers import MindMapTemplateSerializer
...
class MindMapTemplateViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """获取查询集"""
        # 系统模板对所有用户可见
        queryset = MindMapTemplate.objects.filter(is_system=True)
    ...
    def use_template(self, request, pk=None):
        ...
        try:
            ...
            # 创建新的思维导图
            from mind_map.models import MindMap
            ...
            from mind_map.serializers import MindMapSerializer
...
class MongoMindMapTemplateViewSet(viewsets.ViewSet):
    ...
    def get_queryset(self):
        """获取查询集"""
        # 系统模板对所有用户可见
        queryset = MongoMindMapTemplate.objects(is_system=True)
        
        # 如果是管理员，显示所有模板
        if self.request.user.is_staff:
            return MongoMindMapTemplate.objects
    ...
    def retrieve(self, request, pk=None):
        """获取单个模板"""
        try:
            template = MongoMindMapTemplate.objects.get(id=pk)
            serializer = MongoMindMapTemplateSerializer(template)
            return Response(serializer.data)
        except MongoMindMapTemplate.DoesNotExist:
