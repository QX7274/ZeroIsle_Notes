...
class EnhancedMemoryManager {
  ...
  async checkMemoryStatus() {
    try {
      ...
      return {
        hasEnoughMemory: memoryInfo.available > 50 * 1024 * 1024, // 至少50MB可用
  ...
  async getComprehensiveMemoryInfo() {
    ...
    // 如果没有获取到有效信息，使用保守估计
    if (available === 0) {
      available = 100 * 1024 * 1024; // 100MB保守估计
      total = 200 * 1024 * 1024;     // 200MB总内存
      used = 100 * 1024 * 1024;      // 100MB已使用
    }
  ...
  async testBufferAllocation() {
    ...
    // 如果能分配50MB，估计有200MB可用
    return maxAllocatable * 4;
  }
