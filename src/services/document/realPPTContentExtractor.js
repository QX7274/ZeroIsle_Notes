...
/**
 * 真正的PPT内容提取器
 * 能够解析PPT文件并提取实际内容，转换为可浏览的格式
 */
class RealPPTContentExtractor {
  constructor() {
  ...
  async parsePPTContent(filePath, fileContent) {
    try {
      console.log('RealPPTContentExtractor: 开始解析PPT文件:', filePath);
      ...
      console.error('RealPPTContentExtractor: 解析失败:', error);
      throw error;
    }
  }
  ...
  async parsePPTX(fileContent, fileName) {
    try {
      console.log('RealPPTContentExtractor: 解析PPTX文件');
      ...
      console.error('RealPPTContentExtractor: PPTX解析失败:', error);
  ...
  async parsePPT(fileContent, fileName) {
    try {
      console.log('RealPPTContentExtractor: 解析PPT文件');
      ...
      console.error('RealPPTContentExtractor: PPT解析失败:', error);
  ...
  async extractPPTXContent(buffer, fileName) {
    try {
      console.log('RealPPTContentExtractor: 提取PPTX内容');
      ...
      console.error('RealPPTContentExtractor: PPTX内容提取失败:', error);
      throw error;
    }
  }
  ...
  async extractPPTContent(buffer, fileName) {
    try {
      console.log('RealPPTContentExtractor: 提取PPT内容');
      ...
      console.error('RealPPTContentExtractor: PPT内容提取失败:', error);
      throw error;
    }
  }
  ...
  extractTextFromPPTXBuffer(buffer) {
    try {
      console.log('RealPPTContentExtractor: 从PPTX buffer提取文本');
      ...
      console.warn('RealPPTContentExtractor: PPTX文本提取失败:', error);
      return {
        text: '演示文稿内容',
        slideCount: 1,
        tableCount: 0,
  ...
  extractTextFromPPTBuffer(buffer) {
    try {
      console.log('RealPPTContentExtractor: 从PPT buffer提取文本');
      ...
      console.warn('RealPPTContentExtractor: PPT文本提取失败:', error);
      return {
        text: '演示文稿内容',
        slideCount: 1,
        tableCount: 0,
  ...
  generateHTMLContent(fileName, slides) {
    let html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; padding: 20px; max-width: 800px; margin: 0 auto;">
        <h1 style="color: #333; margin-bottom: 20px; text-align: center;">演示文稿: ${fileName}</h1>
...
module.exports = { default: new RealPPTContentExtractor() };
