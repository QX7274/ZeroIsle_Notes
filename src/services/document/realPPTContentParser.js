...
/**
 * 真正的PPT内容解析器
 * 用于提取PPT文件中的实际内容，包括文本、图片、表格等
 */
class RealPPTContentParser {
  constructor() {
  ...
  async parsePPTContent(filePath, fileContent) {
    try {
      console.log('RealPPTContentParser: 开始解析PPT文件:', filePath);
      ...
      console.error('RealPPTContentParser: 解析失败:', error);
      throw error;
    }
  }
  ...
  async parsePPTX(fileContent, fileName) {
    try {
      console.log('RealPPTContentParser: 解析PPTX文件');
      ...
      console.error('RealPPTContentParser: PPTX解析失败:', error);
  ...
  async parsePPT(fileContent, fileName) {
    try {
      console.log('RealPPTContentParser: 解析PPT文件');
      ...
      console.error('RealPPTContentParser: PPT解析失败:', error);
  ...
  async extractPPTXContent(buffer, fileName) {
    try {
      console.log('RealPPTContentParser: 提取PPTX内容');
      ...
      console.error('RealPPTContentParser: PPTX内容提取失败:', error);
      throw error;
    }
  }
  ...
  async extractPPTContent(buffer, fileName) {
    try {
      console.log('RealPPTContentParser: 提取PPT内容');
      ...
      console.error('RealPPTContentParser: PPT内容提取失败:', error);
      throw error;
    }
  }
  ...
  extractTextFromPPTXBuffer(buffer) {
    try {
      console.log('RealPPTContentParser: 从PPTX buffer提取文本');
      ...
      console.warn('RealPPTContentParser: PPTX文本提取失败:', error);
      return {
        text: '演示文稿内容',
        slideCount: 1,
        tableCount: 0,
  ...
  extractTextFromPPTBuffer(buffer) {
    try {
      console.log('RealPPTContentParser: 从PPT buffer提取文本');
      ...
      console.warn('RealPPTContentParser: PPT文本提取失败:', error);
      return {
        text: '演示文稿内容',
        slideCount: 1,
        tableCount: 0,
...
export default new RealPPTContentParser();
