...
class CodeService {
  ...
  async detectCode(text) {
    try {
      const response = await apiService.post('/code/detect', {
        text,
      });
      
      analyticsService.trackCodeAction('detect_code', {
        textLength: text.length,
        codeCount: response.data.codeBlocks.length,
      });
  ...
  async completeCode(code, language) {
    try {
      const response = await apiService.post('/code/complete', {
        code,
        language,
      });
  ...
  async explainCode(code, language) {
    try {
      const response = await apiService.post('/code/explain', {
        code,
        language,
      });
  ...
  async generateExample(language, concept) {
    try {
      const response = await apiService.post('/code/example', {
        language,
        concept,
      });
  ...
  async formatCode(code, language) {
    try {
      const response = await apiService.post('/code/format', {
        code,
        language,
      });
  ...
  async lintCode(code, language) {
    try {
      const response = await apiService.post('/code/lint', {
        code,
        language,
      });
  ...
  async runCode(code, language, input) {
    try {
      const response = await apiService.post('/code/run', {
        code,
        language,
        input,
      });
  ...
  async saveSnippet(code, language, title, description) {
    try {
      const response = await apiService.post('/code/snippets', {
        code,
        language,
        title,
        description,
  ...
  async getSnippets(page = 1, limit = 10) {
    try {
      const response = await apiService.get('/code/snippets', {
        params: { page, limit },
      });
