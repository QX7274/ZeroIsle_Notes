...
// API配置 - 当前未部署生产域名，平板/本机联调优先指向已验证通过的业务后端(8001)
// 如需切换可通过环境变量覆盖：
// 1. ZEROISLE_API_URL 直接覆盖完整基地址
// 2. ZEROISLE_API_HOST / ZEROISLE_API_PORT 覆盖主机与端口
const LOCAL_API_URL_OVERRIDE = process.env.ZEROISLE_API_URL;
const LOCAL_API_HOST = process.env.ZEROISLE_API_HOST || '127.0.0.1';
const LOCAL_API_PORT = process.env.ZEROISLE_API_PORT || '8001';
const LOCAL_API_URL = LOCAL_API_URL_OVERRIDE || `http://${LOCAL_API_HOST}:${LOCAL_API_PORT}`;
