...
| GAP-TEST-001 | 4 | P0 | BLOCKED | 后端/安全智能体 | 测试环境不得强依赖真实外部 Mongo，需改成本地/容器/in-memory | `manage.py check --settings=backend.settings.testing` 不再强连 `127.0.0.1:27017` | `待提交` | 现状证据：`manage.py check` 因 Mongo 强连接失败 | 当前是后续测试治理的主要阻塞之一 |
