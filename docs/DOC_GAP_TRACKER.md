...
| GAP-GROUP-008 | 5 | P1 | IN_PROGRESS | 移动端/Android 智能体 | 收口共享观看态的自动回收、会话快照同步、共享链状态隔离与联调可观测性，避免共享结束、房主离开或无关请求失败后页面停留在假观看态/误报错，并提升真实联调问题定位效率 | 观看端在房主离开、共享会话结束或共享列表刷新后目标会话消失时能够自动退出观看态；观看中的共享对象在刷新后会同步为最新快照；共享页只消费屏幕共享专属 `loading/error`，不再被群组其它请求误禁用或误报错；观看页展示信令连接状态、错误提示与 12 秒远端流等待超时反馈；目标文件 lint 通过 | `ffe9d5f, 11195e5, f41f2ff` | `node node_modules/eslint/bin/eslint.js src/services/webrtc/webrtcService.js src/components/groups/ScreenShare.js src/screens/groups/ScreenShareScreen.js`；`src/services/webrtc/webrtcService.js`；`src/components/groups/ScreenShare.js` | 当前仍缺 Android 专用 MCP 真机可视化验证与 WebRTC 端到端证据；本轮继续补共享链联调可观测性，减少真实联调时“已连上但没流”“信令失败”“远端未推流”混成同一种症状的返工 |
