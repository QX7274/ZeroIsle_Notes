...
| GAP-CI-001 | 4 | P0 | IN_PROGRESS | 部署/CI 智能体 | 让 `mypy`、前端 `lint`、构建检查全部 hard-fail | 不存在 `|| true`、空脚本或 `echo` 伪检查；关键检查失败时流水线失败 | `fba2e33` | `package.json` 脚本差异；`jest --config e2e/jest.config.js --listTests`；Detox 构建日志；前端工具链诊断日志 | 已完成 CLI 直连、Detox CLI 直连与 Detox 20 Jest 模板适配；当前主阻塞收敛为 Android NDK/CMake 原生构建错误，而非脚本入口错误 |
