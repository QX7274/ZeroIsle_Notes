...
| GAP-CI-001 | 4 | P0 | IN_PROGRESS | 部署/CI 智能体 | 让 `mypy`、前端 `lint`、构建检查全部 hard-fail | 不存在 `|| true`、空脚本或 `echo` 伪检查；关键检查失败时流水线失败 | `0f95407` | `package.json` 脚本差异；`yarn bundle:verify` 重跑结果；前端工具链诊断日志 | 已修复 CLI 调用失效问题；当前剩余阻塞是 Metro 打包长耗时，仍需补形成功产物证据 |
