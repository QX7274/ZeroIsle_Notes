...
| GAP-GROUP-008 | 5 | P1 | IN_PROGRESS | 移动端/Android 智能体 | 收口共享观看态的自动回收、会话快照同步、共享链状态隔离与联调可观测性，避免共享结束、房主离开或无关请求失败后页面停留在假观看态/误报错，并提升真实联调问题定位效率 | 观看端在房主离开、共享会话结束或共享列表刷新后目标会话消失时能够自动退出观看态；观看中的共享对象在刷新后会同步为最新快照；共享页只消费屏幕共享专属 `loading/error`，不再被群组其它请求误禁用或误报错；观看页展示信令连接状态、错误提示与 12 秒远端流等待超时反馈；页面内联调摘要可展示当前角色、共享会话 ID、WebRTC 房间、连接详情、远端流状态与最近关键事件，并支持复制摘要；`localhost:8081/shared-screen-lab/` 具备稳定可访问的本地共享链联调壳；目标文件 lint 通过 | `ffe9d5f, 11195e5, f41f2ff, 457e1be, 3e961da` | `node node_modules/eslint/bin/eslint.js src/services/webrtc/webrtcService.js src/components/groups/ScreenShare.js src/screens/groups/ScreenShareScreen.js`；`src/services/webrtc/webrtcService.js`；`src/components/groups/ScreenShare.js`；`node --check scripts/serve-local-web.js`；`ZEROISLE_LOCAL_WEB_PORT=8091` 下 `shared-screen-lab` 返回 200 且命中关键文案 | 当前仍缺 Android 专用 MCP 真机可视化验证与 WebRTC 端到端证据；本轮继续补共享链联调可观测性，并在本地联调入口未完全打通前先用页面内证据面板、摘要复制和 `shared-screen-lab` 诊断页降低“已连上但没流”“信令失败”“远端未推流”混成同一种症状的盲查成本；该能力不等同于真机验证完成 |
