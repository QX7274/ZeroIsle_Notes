...
| GAP-GROUP-008 | 5 | P1 | IN_PROGRESS | 移动端/Android 智能体 | 收口共享观看态的自动回收、会话快照同步与共享链状态隔离，避免共享结束、房主离开或无关请求失败后页面停留在假观看态/误报错 | 观看端在房主离开、共享会话结束或共享列表刷新后目标会话消失时能够自动退出观看态；观看中的共享对象在刷新后会同步为最新快照；共享页只消费屏幕共享专属 `loading/error`，不再被群组其它请求误禁用或误报错；目标文件 lint 通过 | `ffe9d5f, 11195e5` | `node node_modules/eslint/bin/eslint.js src/redux/slices/groupsSlice.js src/components/groups/ScreenShare.js src/screens/groups/ScreenShareScreen.js`；`src/redux/slices/groupsSlice.js`；`src/components/groups/ScreenShare.js` | 当前仍缺 Android 专用 MCP 真机可视化验证与 WebRTC 端到端证据；本轮继续补共享链状态隔离，减少真实联调时出现“共享按钮被无关请求锁死”或“邀请/详情错误串到共享页”的返工 |
