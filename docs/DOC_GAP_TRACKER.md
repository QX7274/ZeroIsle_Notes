...
| GAP-CI-001 | 4 | P0 | IN_PROGRESS | 部署/CI 智能体 | 让 `mypy`、前端 `lint`、构建检查全部 hard-fail | 不存在 `|| true`、空脚本或 `echo` 伪检查；关键检查失败时流水线失败 | `待提交` | `package.json` 脚本差异；`bundle:verify` 重跑结果；前端工具链诊断日志 | 已定位当前环境下 `react-native` CLI 不能依赖 PATH 解析，需先修复脚本调用路径再恢复稳定门禁证据 |
...
| GAP-MOBILE-001 | 4 | P1 | IN_PROGRESS | 移动端/Android 智能体 | 补齐关键页面稳定 `testID` 与 Detox smoke 基础 | 首页、底部导航、AI、提醒、知识图谱、我的页具备稳定 `testID`；smoke 可构建安装 | `待提交` | 首轮 Detox/`testID` 差异；`detoxrc.js`；`e2e/starter.test.js`；依赖核查结果 | 已确认 smoke 用例与配置存在，但当前主应用侧 `detox` 依赖未解析到，需补齐安装链后才能进入构建验证 |
