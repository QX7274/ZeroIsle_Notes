...
| GAP-CI-001 | 4 | P0 | IN_PROGRESS | 部署/CI 智能体 | 让 `mypy`、前端 `lint`、构建检查全部 hard-fail | 不存在 `|| true`、空脚本或 `echo` 伪检查；关键检查失败时流水线失败 | `待提交` | `package.json` 脚本差异；`jest --config e2e/jest.config.js --listTests`；Detox 构建日志；前端工具链诊断日志 | 已完成 CLI 直连、Detox CLI 直连与 Detox 20 Jest 模板适配；当前主阻塞收敛为 Android NDK/CMake 原生构建错误，而非脚本入口错误 |
...
| GAP-MOBILE-001 | 4 | P1 | IN_PROGRESS | 移动端/Android 智能体 | 补齐关键页面稳定 `testID` 与 Detox smoke 基础 | 首页、底部导航、AI、提醒、知识图谱、我的页具备稳定 `testID`；smoke 可构建安装 | `待提交` | `detoxrc.js`；`e2e/starter.test.js`；`android/app/build.gradle`；`android/settings.gradle`；`android/app/src/androidTest/java/com/zeroisle_notes/DetoxTest.java`；页面 `testID` 差异 | 已补 attached-device 配置、Instrumentation 入口与页面关键锚点；构建已推进到 `react-native-reanimated` 原生链接阶段，当前失败并非 `testID` 缺口所致 |
| GAP-REVIEW-001 | 5 | P0 | IN_PROGRESS | 验证智能体 | 使用工具进行功能审查，核查规划功能是否完成、是否足以上线 | 形成逐模块核查结论、缺口清单、体验与速度评估 | `待提交` | smoke 静态审查结论；`jest --config e2e/jest.config.js --listTests`；Detox attached-device 错误日志；后续截图与日志 | 已将 smoke 扩展到 AI 输入、提醒创建页、知识分析页最短闭环；当前运行阻塞已收敛为 `ANDROID_SDK_ROOT` 缺失与 Android 构建产物未生成 |
| GAP-TOKEN-001 | 4/5 | P1 | IN_PROGRESS | 总控智能体 | 将 RTK 纳入默认 shell 巡检与验证策略，降低多轮整改过程中的 token 消耗 | 总控文档与台账明确 RTK 规则；后续常规搜索、读文件、diff、测试输出默认优先使用 `rtk` | `待提交` | `C:\\Users\\QX\\.codex\\AGENTS.md`；`C:\\Users\\QX\\.codex\\RTK.md`；本地 `D:\\rtk` 目录核查；本轮低 token 巡检记录 | 仅在 RTK 不支持目标命令或需要完整原始输出时回退原生命令，并在日志中说明原因；本轮已按 RTK/最小上下文原则收敛 Detox 与页面锚点整改范围 |
